...
public class DatabaseFiller {
	...
	private static DatabaseContent __fillDatabase(
		...
		try {
			...
			info("An error occured while persisting the entities: %s", e);
			throw e;
		}
		info("done\n");
	...
	private static void __writeSystemConfiguration(
		...
		int storeNumber = 0;
		for (final IStore store : content.stores) {
	...
	private static void __collectItemInfos(
			final List<IStockItem> stockItems, final ItemType itemType,
	...
	static File getOutputDir(String dirString) {
		if (!dirString.trim().isEmpty()) {
			final File outputDir = new File(dirString);
			info(Boolean.toString(new File(".").isDirectory()));
			if (!outputDir.isDirectory()) {
				info("Invalid output directory: %s\n", outputDir);
				return null;
			}
