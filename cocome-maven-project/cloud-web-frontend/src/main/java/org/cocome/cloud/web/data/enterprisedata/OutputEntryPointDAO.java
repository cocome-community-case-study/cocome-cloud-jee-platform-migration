...
@Named
@SessionScoped
public class OutputEntryPointDAO extends AbstractEnterpriseDAO<EntryPointTO> {
    ...
    protected Collection<EntryPointTO> queryAllByParentObj(IEnterpriseManager iEnterpriseManager, long plantId) throws NotInDatabaseException_Exception {
        return iEnterpriseManager.queryOutputEntryPointsByRecipeOperationId(plantId);
    }

    @Override
    public ViewData<EntryPointTO> createViewDataInstance(EntryPointTO target) {
        target.setDirection(EntryPointTO.DirectionTO.OUTPUT);
