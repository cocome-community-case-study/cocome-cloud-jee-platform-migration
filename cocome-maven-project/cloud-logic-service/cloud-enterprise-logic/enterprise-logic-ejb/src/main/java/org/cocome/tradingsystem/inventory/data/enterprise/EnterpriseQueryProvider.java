...
public class EnterpriseQueryProvider implements IEnterpriseQueryLocal {
	...
	public ITradingEnterprise queryEnterpriseById(long enterpriseID) throws NotInDatabaseException {
		...
		try {
			...
			ITradingEnterprise enterprise = csvHelper.getEnterprises(
	 ...
	 */
	@Override
	public long getMeanTimeToDelivery(IProductSupplier supplier,
	...
	@Override
	public ITradingEnterprise queryEnterpriseByName(String enterpriseName)
	...
	@Override
	public Collection<IProduct> queryProductsBySupplier(long enterpriseID,
			long productSupplierID) throws NotInDatabaseException {
		ITradingEnterprise enterprise = queryEnterpriseById(enterpriseID);
		
		for (IProductSupplier supplier : enterprise.getSuppliers()) {
			if (supplier.getId() == productSupplierID) {
				return csvHelper.getProducts(
	...
	public Collection<IStore> queryStoresByEnterpriseId(long enterpriseID) {
		Collection<IStore> stores = csvHelper.getStores(
	...
	public Collection<ITradingEnterprise> queryAllEnterprises() {
		Collection<ITradingEnterprise> enterprises = csvHelper.getEnterprises(
				backendConnection.getEnterprises("name=LIKE%20'*'"));
		return enterprises;
	}

	@Override
	public IStore queryStoreByEnterprise(long enterpriseID, long storeID)
			throws NotInDatabaseException {
		try {
	...
	@Override
	public IProductSupplier querySupplierByID(long supplierID)
			throws NotInDatabaseException {
		try {
			IProductSupplier supplier = csvHelper.getProductSuppliers(
	...
	@Override
	public IProduct queryProductByID(long productID)
			throws NotInDatabaseException {
		try {
	...
	public Collection<IProduct> queryAllProducts() {
		Collection<IProduct> products = csvHelper.getProducts(
