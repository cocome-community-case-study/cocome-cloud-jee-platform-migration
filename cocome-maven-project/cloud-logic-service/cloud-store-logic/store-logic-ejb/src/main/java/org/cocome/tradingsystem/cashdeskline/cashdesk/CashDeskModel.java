...
public class CashDeskModel implements Serializable, ICashDeskModel {
    ...
    private void addItemToSale(ProductWithItemTO product) throws ProductOutOfStockException {
        if (product.getItem() instanceof StockItemTO) {
            checkSufficientProductStockAmount((StockItemTO) product.getItem());
        }
        ...
        final double price = product.getItem().getSalesPrice();

        this.runningTotal = this.computeNewRunningTotal(price);

        this.sendRunningTotalChangedEvent(product.getProduct().getName(), price);
    }

    private void checkSufficientProductStockAmount(
            StockItemTO stockItem)
            throws ProductOutOfStockException {
        long currentAmount = stockItem.getAmount();
        for (final SaleEntryTO entry : saleProducts) {
            if (entry.getItemInfo().getItem().getId() == stockItem.getId()) {
                final StockItemTO savedItem = (StockItemTO) entry.getItemInfo().getItem();
                currentAmount = savedItem.getAmount();
                if (currentAmount > 0) {
                    savedItem.setAmount(currentAmount - 1);
                }
            }
        }
    ...
    private void makeSale(final PaymentMode mode) {
        ...
        //
        this.sendSaleRegisteredEvent(saleTO.getEntries().size(), mode);
    }

    private SaleTO createSaleTO() {
        final SaleTO saleTO = new SaleTO();
        saleTO.setDate(new Date());
        saleTO.setEntries(this.saleProducts);
        return saleTO;
    }
     ...
     *
     * @param elements number of states
